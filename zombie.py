...
class Zombie:
    ...
    def handle_collision(self, group, other):
        if group == 'zombie:ball':
            self.ball_count += 1
                                ...
                                #  1000    1000  -> 1번에서 값이 넘어옴
    def set_target_location(self, x=None, y=None): # action -> 성공/실패를 리턴해야함
        # 3
    ...
    def distance_less_than(self, x1, y1, x2, y2, r): # r보다 작은지 -> r은 미터단위
        # 4-3
        distance2 = (x1 - x2) ** 2 + (y1 - y2) ** 2
        return distance2 < (r * PIXEL_PER_METER) ** 2 # 디스턴스는 픽셀단위 -> 미터 단위 거리 비교이므로 제곱해서 비교
        pass
    ...
    def move_to(self, r=0.5):
        ...
        if self.distance_less_than(self.tx, self.ty, self.x, self.y, r):
            return BehaviorTree.SUCCESS # 목적지 도착
        else:
    ...
    def get_patrol_location(self): # 다움 순찰 위치 획득
        # 11
    ...
    def build_behavior_tree(self):
        ...
        chase_or_wander = Selector('추적 아니면 배회', chase_boy, wander)

        #12
        a5 = Action('다음 순찰 위치 획득', self.get_patrol_location)
        patrol = Sequence('순찰', a5, a2)

        root = patrol_or_chase = Selector('추적 아니면 순찰', chase_boy, patrol)
